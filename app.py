...
def upload():
    ...
    x_train = np.column_stack(( et_oof_train, rf_oof_train, ada_oof_train, gb_oof_train))
    x_test = np.column_stack(( et_oof_test, rf_oof_test, ada_oof_test, gb_oof_test))

    gbm = xgb.XGBRegressor(n_estimators= 2000).fit(x_train, y_train)
